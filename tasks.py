...
def get_dir():
	if WINDOWS:
		#print os.path.dirname(__file__)+ '\\lists\\'
...
class Task:
	...
	def __unicode__(self):
		...
		if not WINDOWS:
			...
			if not topic:
				topic = "None"
			#return "<{}> [{}] {} [{}]".format(os.path.splitext(os.path.basename(self.file))[0], topic.encode("cp437"), self.name.encode('cp437'), self.planned_time_to_str().encode('cp437'))
			return "<{}> [{}] {} [{}]".format(os.path.splitext(os.path.basename(self.file))[0], topic.encode("utf-8"), self.name.encode('utf-8'), self.planned_time_to_str().encode('utf-8'))
